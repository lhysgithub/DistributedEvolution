...
def main():
    ...
    with tf.Session() as sess:
        # 完成输入
        InputImg = tf.constant(TargetImg,dtype=tf.float32) #（299，299，3）
        ...
        ### 筛选在目标分类的扰动
        InputTestImage = tf.placeholder(shape=IndividualShape,dtype=tf.float32) # （BatchSize，299，299，3）
        ITIC, ITIP = model(sess, InputTestImage)  # TMD 为啥在我的程序里这个model不好使了
        ### 筛选在目标分类的扰动
        ...
        def render_frame(sess, image, save_index):
            ...
            image = np.reshape(image,(299,299,3))+TargetImg
            # testmax = np.max(image)
            # testmin = np.min(image)
        ...
        for i in range(MaxEpoch):
            ...
            while count != BatchSize:
                ...
                testimage = temp + TargetImg
                ITICnp, ITIPnp = sess.run([ITIC, ITIP],{InputTestImage:testimage})
                for j in range(BatchSize):
                    if ITIPnp[j] == TargetClass:
                        initI[count] = temp[j]
                        count += 1
                        if count == BatchSize:
                            break
