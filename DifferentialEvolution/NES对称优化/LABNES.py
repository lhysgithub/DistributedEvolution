...
def main():
    ...
    with tf.Session() as sess:
        ...
        noiseP = tf.placeholder(dtype=tf.float32,shape=(batch_size,299,299,3))
        eval_points = x_t + SIGMA * noiseP
        logits, preds = model(sess, eval_points)
        alllosses = tf.nn.softmax_cross_entropy_with_logits(logits=logits, labels=labels)
        ...
        # 分割线
        GreatTImages = tf.placeholder(dtype=tf.int32,shape=(batch_size,1))
        GreatImages = tf.where(tf.equal(GreatTImages, 1))
        GreatImages = GreatImages[:,0]
        temploss = tf.placeholder(dtype=tf.float32,shape=(batch_size))
        tempnoise = tf.placeholder(dtype=tf.float32,shape=(batch_size,299,299,3))
        losses = tf.gather(temploss, GreatImages)
        noise = tf.gather(tempnoise, GreatImages)
        ...
        def get_grad(pt,InitImg,TargetImg,TargetClass,Labels,lower,upper,meannoise,stdnoise,should_calc_truth=False):
            global UnVaildExist
            ...
            for _ in range(num_batches):
                Noise = np.random.randn(batch_size//2,299,299,3)
                Noise = np.concatenate((Noise,-Noise),axis=0)
                Noise *= stdnoise
                GreatInds = np.zeros(batch_size)

                feed_dict = {x: pt, Init_Img: InitImg, Target_Img: TargetImg, Target_Class: TargetClass, labels: Labels, noiseP : Noise}
                Allloss,TempInds,logitsnp = sess.run([alllosses,good_images,logits], feed_dict)
                for i in TempInds:
                    if i+batch_size//2<batch_size and i+batch_size//2 in TempInds:
                        GoodInds.append(i)
                        GoodInds.append(i+batch_size//2)
                        UsedCount +=2
                # for i in range(batch_size):
                #     if TargetClass in logitsnp[i].argsort()[-k:][::-1]:
                #         tempnoise.append(Noise[i])
                #         count+=1
                for i in GoodInds:
                    GreatInds[i] = int(1)
                GreatInds =np.reshape(GreatInds,(UsedCount,1))
                # GoodInds = GoodInds[:,0]
                loss, dl_dx_ = sess.run([finallosses, grad_estimate], {temploss:Allloss,tempnoise:Noise,GreatTImages:GreatInds})
                losses.append(np.mean(loss))
                grads.append(dl_dx_)
            if count< samples_per_draw*0.8:
                UnVaildExist = 1
            else:
                UnVaildExist = 0
            meannoise = np.mean(tempnoise,axis=0)
            stdnoise = np.sqrt(np.sum(np.square(tempnoise - meannoise),axis=0)/np.shape(tempnoise)[0])
            print("count: %d UsedCount: %d"%(count,UsedCount))
            return np.array(losses).mean(), np.mean(np.array(grads), axis=0),meannoise,stdnoise
        ...
        for p in range(1,100):
            ...
            for i in range(max_iters):
                ...
                prev_g = g
                l, g,mnoise,stnoise = get_grad(adv,InitImg,TargetImg,TargetClass,Labels,lower,upper,mnoise,stnoise)
                ...
                if UnVaildExist ==0 :
                    ...
                    adv = adv + SIGMA*mnoise
                    print('Change adv to meannoise')
