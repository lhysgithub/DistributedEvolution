...
from TencentAPI import Predict
...
def main():
    ...
    with tf.Session() as sess:
        ...
        def render_frame(sess, image, save_index):
            ...
            probs,_ = Predict(np.reshape(image,(1,299,299,3)))
            probs = np.array(np.reshape(probs,(1000)))
            # classifications
        ...
        for i in range(MaxEpoch):
            ...
            newimage = initI + SourceImg
            CP,PP = Predict(newimage)
            ENP,DNP,PBF,PB = sess.run([Expectation,StdDeviation,PbestFitness,Pbest],feed_dict={Individual:initI,Confidenceplaceholder:CP,Predictiondsplaceholder:PP})
            if PBF > GBF:
                GB = PB
                GBF = PBF
                # GENP = ENP
