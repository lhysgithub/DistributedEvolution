...
def main():
    ...
    with tf.Session() as sess:
        ...
        for T in range(0,50):
            ...
            Convergence = 0.2
            CloseThreshold = - (0.5 - 0.01 * T)  # 置信度靠近条件
            # if T==0:
            #     CloseThreshold = - 0.5  # 置信度靠近条件
            # else :
            #     CloseThreshold = - (0.5-0.05*T)  # 置信度靠近条件
            ...
            for p in range(0, 4):
                ...
                if TClass == SClass:
                    ...
                    LogFile = open(os.path.join(OutDir, 'log%d-%d.txt' %(T,p)), 'w+')
                ...
                for i in range(MaxEpoch):
                    ...
                    if BestAdvL2 < 26 and BestAdvL2 + BestAdvF > CloseThreshold:
                        ...
                        print(LogText)
                        LogFile.write(LogText + '\n')
                        render_frame(sess, BestAdv, T*100+p, SClass, TClass, StartImg)
                        break
