...
def main():
    ...
    with tf.Session() as sess:
        ...
        TestNewImage =TestImge
        TestSourceImg =tf.placeholder(shape=ImageShape, dtype=tf.float32)
        TestLabel = tf.placeholder(dtype=tf.int32, shape=(1, 1000))
        TestL2Distance = tf.sqrt(tf.reduce_sum(tf.square(TestNewImage - TestSourceImg), axis=(0, 1, 2)))
        TestFitness = - (Sigma * tf.nn.softmax_cross_entropy_with_logits(logits=TestC, labels=TestLabel) + TestL2Distance)
        # ########################################render_frame
            ...
            for i in range(MaxEpoch):
                ...
                ENP, DNP, PBF, PB = sess.run([Expectation, StdDeviation, PbestFitness, Pbest],
                                             feed_dict={Individual: initI, logit: initCp, SourceImg: SImg,
                                                        ...
                                                        StImg: StartImg})
                LastENPF,LastENPL2 = ENPF,ENPL2
                ENPF,ENPL2 = sess.run([TestFitness,TestL2Distance],{TestImge:StartImg+ENP,TestLabel:np.expand_dims(OHV, axis=0),TestSourceImg:SImg})
                ...
                LogText = "Step %05d: ENPF: %.4f UseingTime: %.4f ENPL2Distance: %.4f QueryTimes: %d P: %d" % (
                    i, ENPF, End - Start, ENPL2,QueryTimes,p)
                LogFile.write(LogText + '\n')
                print(LogText)
                ...
                elif abs(ENPF - LastENPF) < Convergence :
                    if (ENPF + ENPL2 > CloseThreshold):  # 靠近
                        Closed = 1
                        Retry = 0
                        Scaling = 0
                        CEV += 0.01
